...
const pathToEXE = isDev
  ? 'release/win-unpacked/resources/downloads/'
  : '../../resources/downloads/';
app.whenReady().then(async () => {
  ...
  let init = exec(
    ...
    )} init -f ipns://k51qzi5uqu5dkz7hakn5fjvwz0vlv55wjlkw2cjb8noukucrpmfo7s6s1scxmh/`,
  ...
  let bootstrap = exec(
    `${path.join(
      process.env.ROOT!,
      `${pathToEXE}ipfs-cluster-service/ipfs-cluster-service.exe`,
    )} -f daemon --bootstrap /ip4/127.0.0.1/tcp/9092/p2p/12D3KooWR8MxQgHGLF5vx1ShZy3Dkb5ZMjgm5MjVpJaDpi1Qqpwp`,
