<template>
  <v-menu class="invite-member" :close-on-content-click="false" location="top" :offset="4">
    <!-- Exibição -->
    ...
    <!-- Opções -->
    <v-list class="list tw-py-0 tw-relative" :selectable="false" :activable="false">
      ...
      <!-- Turmas -->
      <p v-if="filteredClasses?.length" class="tw-pa-4 tw-pb-2 lines-1 ellipsis text-body-4 text-gray-800">
      ...
      >
        <v-list-item-title
          ><div class="d-flex align-center ga-4">
            <v-avatar :size="40" class="alex-avatar-group-border alex-avatar-group-margin" color="gray-100">
              <p class="text-gray-300 text-body-2">
                {{ getInitials(studentClass.name) }}
              </p>
            </v-avatar>
            <p class="text-body-4 text-gray-900 lines-1 ellipsis">
              {{ studentClass.name || $t('components.learningPlan.members.missing.name') }}
            </p>
      ...
      <div v-for="(studentClass, index) in filteredClassMembers" :key="index">
        ...
        >
          {{ $t('components.learningPlan.members.invite.members') }} ({{ studentClass.name }})
        </p>

        <template v-if="studentClass.learning_plan_members?.length">
          <v-list-item
            v-for="(student, studentIndex) in studentClass.learning_plan_members"
          ...
          >
            <v-list-item-title
              ><div class="d-flex align-center ga-4">
                ...
                <div>
                  <p class="text-body-4 text-gray-900 lines-1 ellipsis">
                    {{ student.user.fullname || $t('components.learningPlan.members.missing.name') }}
                  </p>
                  <p class="text-body-5 text-gray-500 lines-1 ellipsis">
                    {{ student.email || $t('components.learningPlan.members.missing.email') }}
                  </p>
                </div>
              </div></v-list-item-title
            >
...
const { data: classes } = await useAsyncData('classes-member-invite', () => getMembers(props.learningplanId));

const filteredClasses = computed(() => {
  if (!classes.value?.data) {
    return [];
  }
  return classes.value.data.filter((classValue) => classValue.name.toLowerCase().includes(search.value));
});
...
const filteredClassMembers = computed(() => {
  ...
  return classes.value.data.map((classValue) => ({
    ...classValue,
    learning_plan_members: filterLearningMembers(classValue.learning_plan_members),
  }));
});

const hasFilteredMember = computed(
  () => filteredClassMembers.value.filter((classValue) => classValue.learning_plan_members?.length).length,
);
