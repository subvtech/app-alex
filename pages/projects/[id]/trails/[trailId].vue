<template>
  <div>
    <alex-project-trails-header
      v-if="trailStore.trail"
      :trails-description="trailStore.trail.description"
      :trails-cover="trailStore.trail.cover_image ? trailStore.trail.cover_image.url : '/images/cover_image_course.svg'"
      :trail-id="trailId"
...
const getTrailData = async () => {
  ...
  if (!(trailStore.trail?.learning_structure.learningplan.id === learningPlanId.value)) {
    navigateTo(`/projects/${learningPlanId.value}/trails`);
  }
};
...
watch(pageRoute, async () => {
  if (pageRoute.value?.toString().includes('projects-id-trails-trailId')) {
    await getTrailData();
  }
});
</script>
